...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'You\'ve been signed in for a while. Please log in again to delete your account.')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Uh-oh! Something went wrong while signing in. Here\'s the scoop: [error]'
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('Boom! Password reset link is flying to your inbox.')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : AttendrixFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' =>
          'Looks like someone’s already using that email. Maybe try logging in?',
        'INVALID_LOGIN_CREDENTIALS' =>
          'That combo doesn’t seem to work. Double-check your email and password.',
        _ =>
          'Uh-oh! Something went wrong while signing in. Here\'s the scoop: [error]'
              .replaceAll('[error]', e.message!),
      };
